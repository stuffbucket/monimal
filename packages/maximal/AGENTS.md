...
| Change routing, middleware, model dispatch, config, the token store, or diagnostics | **Wrong package** — that is the engine, and it lives in [`stuffbucket/maximal-core`](https://github.com/stuffbucket/maximal-core). This package's `src/` is composition only; see [`docs/architecture.md`](docs/architecture.md). |
| Change packaging, the published CLI, or the site                                    | [`docs/architecture.md`](docs/architecture.md)                                                                                                                                                                                    |
| Write, change, or run tests                                                         | [`docs/commands.md`](docs/commands.md), then [`docs/architecture.md`](docs/architecture.md) → _Testing_                                                                                                                           |
...
- **The proxy engine is not in this package.** Routing, middleware, model dispatch, config, the token store, and diagnostics all live in [`stuffbucket/maximal-core`](https://github.com/stuffbucket/maximal-core). `src/` may compose public package seams but must not reimplement engine or provider behavior.
