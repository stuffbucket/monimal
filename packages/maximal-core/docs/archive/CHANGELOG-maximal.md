# Changelog — stuffbucket/maximal (pre-split)

**This is not this repository's changelog.** It is the release history of
[`stuffbucket/maximal`](https://github.com/stuffbucket/maximal), which
`maximal-core` was split out of in August 2026 (`ced18dd`). It came across in
the split and covers `0.4.10` through `0.4.41` (2026-07-10) — every version,
compare link, commit link and issue link below points at the **parent** repo,
and none of it describes a `maximal-core` release.

It is kept because the entries are still the accurate history of the code that
became this package, and because deleting them would strand the links that
reference them. It is **frozen**: nothing gets appended here.

This repository's own changelog starts at
[`CHANGELOG.md`](../../CHANGELOG.md).

`scripts/ops/release-notes.test.ts` reads this file as a format fixture, since
a frozen file is a stable one — the live changelog's top entry changes with
every release.
