...
# Amendment (2026-08-31): the committed `dist/` bundle is a third-party alert surface

`packages/maximal-core/dist/` is committed (~75k lines) because
`bindings:check` verifies that the checked-in bundle matches a fresh
build — that is what makes the package consumable as a git dependency,
which has no publish step to build during. The bundle is produced by
inlining `node_modules`, so every dependency's source becomes repo
content that CodeQL analyzes as if we wrote it.

Seven alerts on PR #25 are entirely that. None is in a workspace `src/`:

| Rule | Bundle line | Origin | Assessment |
|---|---|---|---|
| `js/incomplete-sanitization` (high) | 4201 | `execa@5.1.1/lib/command.js` | `escapeArg` feeds only `getEscapedCommand`, which builds the human-readable `escapedCommand` field on execa's result and error objects. execa spawns through `cross-spawn` with an argv array and never a shell, so this is a display formatter, not a sanitizer. False positive |
| `js/incomplete-sanitization` (high, ×2) | 56354, 56358 | `turndown@7.2.4` | `escapeLinkDestination` / `escapeLinkTitle` escape markdown for output. Not a security boundary, and any real gap is upstream's to close. Won't fix |
| `js/overly-large-range` (medium, ×4) | 72250–72251 | `hono@4.13.2/dist/utils/cookie.js` | `[!#-:<>-[\]-~]` is the RFC 6265 cookie-name / cookie-octet charset, written deliberately as ranges. False positive |

## Why none of this ADR's three existing mechanisms applies

**Inline `// codeql[...]` cannot survive.** The bundle is regenerated by
the build from `node_modules` on every commit that touches Core. A
comment added by hand is erased on the next build, and `bindings:check`
then fails on the diff between the annotated file and the fresh one.
The mechanism this ADR prefers is load-bearing precisely because the
comment travels with source we own; it has nothing to attach to here.

**`query-filters` has no config to live in.** monimal runs CodeQL
**default setup** (`GET /repos/stuffbucket/monimal/code-scanning/default-setup`
returns `state: configured`, `query_suite: default`), which does not read
an in-repo configuration file. The `links.codeql_config` frontmatter
above — `.github/codeql/codeql-config.yml` — is inherited from the
`stuffbucket/maximal` repo; that path does not exist in monimal, and
creating it would be inert. A rule-wide exclusion would also be the
wrong tool: both rules should keep running against workspace `src/`.

**So the alerts are dismissed per-alert, and the rationale lives here.**
This is the case the original Context anticipated — "the rationale is
per-alert and not greppable" — resolved the only way left: the Security
tab holds the dismissal, this table holds the why.

## Consequences

- Code scanning keys dismissals on CodeQL fingerprints computed from
  surrounding context, not line numbers, so ordinary bundle churn that
  shifts these lines does not reopen them.
- A dependency upgrade that rewrites one of the flagged functions *will*
  reopen its alert as new. That is correct behavior: the third-party
  code changed and the assessment above is no longer evidence about it.
- The structural alternative is advanced setup with `paths-ignore:
  packages/maximal-core/dist/**`, which would end this class of alert
  permanently. It is deliberately not taken: it costs an owned
  `codeql.yml` plus a repository-settings migration off default setup,
  and it would silence the bundle wholesale rather than one reviewed
  finding at a time. Revisit if the dismissal list grows past a handful.
