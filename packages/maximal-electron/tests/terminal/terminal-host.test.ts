...
describe('TerminalHost connector', () => {
  ...
  it('transfers a live process and routes retained and later output to the destination', async () => {
    ...
    const destinationOutput: string[] = [];
    const source = new TerminalHost({
      homeDirectory: '/home/test',
      defaultShell: '/bin/sh',
      connector: { connect: () => process },
      emit: (_id, chunk) => sourceOutput.push(chunk),
      onExit: () => undefined,
    });
    const destination = new TerminalHost({
      homeDirectory: '/home/test',
      defaultShell: '/bin/sh',
      connector: { connect: () => { throw new Error('must not reconnect'); } },
      emit: (_id, chunk) => destinationOutput.push(chunk),
      onExit: () => undefined,
    });

    source.spawn({ id: 'one', cols: 80, rows: 24 });
    dataListener('buffered before transfer');
    expect(source.transfer('one', destination, { id: 'one', cols: 100, rows: 30 })).toBe(true);
    expect(source.list()).toEqual([]);
    expect(destination.list()).toHaveLength(1);
    expect(destination.list()[0]?.id).toBe('one');
    destination.spawn({ id: 'one', cols: 100, rows: 30 });
    await Promise.resolve();
    expect(destinationOutput).toEqual(['buffered before transfer']);
    dataListener('still live');
    exitListener({ exitCode: 0 });

    expect(sourceOutput).toEqual([]);
    expect(destinationOutput).toEqual(['buffered before transfer', 'still live']);
